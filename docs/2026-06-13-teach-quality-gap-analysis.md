...
### 2026-06-24 模块 13：P4 教学质量服务接入 fileObjectId 绑定

- 分支：`feat/p4-quality-fileobject-binding`
- 范围：`micro-teaching-quality`、`micro-teaching-file`

本次完成：
- 为独立文件服务补上了文件下载接口，业务服务现在不只可以查文件元数据，也可以直接通过文件服务拉取文件流。
- 在 `micro-teaching-quality` 新增 `FileObjectReferenceService`，统一处理：
  - `fileObjectId` 引用格式生成与解析
  - 远程查询文件对象
  - 远程删除文件对象
  - 远程代理下载文件
- 为课程评审、专业评审各新增一套“绑定已上传文件对象”的入口：
  - 课程评审：新增 `principalBind`
  - 专业评审：新增 `principalBind`
- 业务侧上传记录改为支持双模式：
  - 老数据继续使用原本的本地加密路径
  - 新数据可直接记录 `file-object:{id}` 形式的远程文件引用
- 下载、预览、删除接口都增加了双模式兼容逻辑：
  - 如果是旧加密路径，继续走原本本地磁盘逻辑
  - 如果是 `file-object:{id}`，则转调独立文件服务完成下载/删除
- 课程评审同名文件覆盖场景增加了远程文件清理，避免原文件已经迁移到文件服务后继续留下孤儿对象。

本次自检重点：
- 这一步已经把“上传文件流”与“业务流程推进”从接口层拆开了，前端后续可以先调文件服务，再把 `fileObjectId` 传给教学质量服务。
- 历史下载/预览接口没有被硬切掉，老数据仍然可读。
- 受当前环境 Maven 远程依赖下载额度限制影响，这一步没能完成完整的联动构建验证；已做静态代码自检，后续在可用额度恢复后需要补跑一次 `micro-teaching-file + micro-teaching-quality` 联合构建。

### 2026-06-24 模块 14：P4 课程评价模板接入 fileObjectId

- 分支：`feat/p4-course-template-fileobject`
- 范围：`CourseReportController`、`CourseReportService`、`CourseReportServiceImpl`

本次完成：
- 为课程评价模板新增了“绑定文件服务文件对象”的接口，学校管理员现在可以不再直接传 `MultipartFile`，而是把已上传完成的 `fileObjectId` 绑定成模板记录。
- 为课程评价模板新增了“引用式更新”能力：
  - 继续保留原有 `MultipartFile` 更新方式
  - 新增基于 `fileObjectId` 的替换模板方式
- 模板下载现在支持双模式兼容：
  - 老模板继续按历史加密本地路径读取
  - 新模板如果记录的是 `file-object:{id}`，则转调文件服务下载
- 模板删除和模板更新都增加了旧文件清理：
  - 旧记录是本地文件时，删除本地文件与目录
  - 旧记录是文件服务引用时，删除远端文件对象

本次自检重点：
- 课程评价模板这条链路现在已经具备和课程评审/专业评审相同的“本地路径 + fileObject 引用”双模式兼容能力。
- 当前这一步仍然没有把更多历史模板/归档接口统一抽成一个公共文件代理组件，后续继续收敛时可以复用这次课程模板的接法。
- 受当前环境限制，这一步同样没有完成完整 Maven 构建验证，已完成静态代码自检。

### 2026-06-24 模块 15：P4 专业归档文件与模板接入 fileObjectId

- 分支：`feat/p4-major-archive-fileobject`
- 范围：`MajorArchiveController`、`MajorArchiveProcessService`、`MajorArchiveProcessServiceImpl`、`MajorArchiveTemplateFileMapper`、相关 MyBatis XML

本次完成：
- 为专业归档“专家上传材料”新增了基于 `fileObjectId` 的绑定入口，前端现在可以先走独立文件服务完成上传，再把文件对象绑定到专业归档批次。
- 为专业归档“负责人上传模板”新增了基于 `fileObjectId` 的绑定入口，模板记录同样支持直接引用独立文件服务中的文件对象。
- 专业归档上传文件下载接口改成双模式兼容：
  - 历史数据继续按 AES 加密本地路径下载
  - 新数据如果记录的是 `file-object:{id}`，则通过文件服务代理下载
- 专业归档模板下载接口改成双模式兼容，和上传材料链路保持一致。
- 专业归档上传文件删除、模板删除都增加了文件对象删除逻辑，避免记录删除后在文件服务里遗留孤儿文件对象。
- 为模板记录补了按 `path` 查询能力，便于删除 `file-object` 引用时先拿到数据库记录。
- 顺手修复了专业归档上传文件删除 SQL 未按 `user_id` 限制的问题，避免仅凭路径就把他人记录标记删除。
- 专业归档这块的本地下载代码收敛到 `DownloadUtil`，减少控制器里重复的流式输出样板代码。

本次自检重点：
- `major_archive_file.path` 和 `major_archive_template_file.path` 继续复用原字段，不做表结构变更，历史本地路径和新 `file-object:{id}` 引用可以共存。
- 当前专业归档这一步已经具备与课程评审、专业评审、课程模板一致的文件服务接入方式，前端接入成本更低。
- 受当前环境 Maven 构建额度/依赖限制影响，这一步没有完成完整编译验证，已完成静态 diff 自检和接口链路复查。
