...
本次自检重点：
- 首次提交评审时会按当前登录用户写入 `MasterEvaluation` 和 `OptionRecord`。
- 已存在评审记录时会先清理旧记录，再写入新记录，保证更新结果覆盖旧结果。
- 更新接口在记录不存在时会明确抛出业务异常，不再静默失败。

待下一步继续：
- 运行针对性检查，确认该服务编译和基本流程无明显回归。
- 推送当前分支后，继续进入下一个 P0 子模块。
