...
import com.vtmer.microteachingmessage.constant.MajorEvaluationUserTypeConstant;
import com.vtmer.microteachingmessage.mapper.MajorEvaluationProcessMapper;
 ...
 */
@Service
public class MajorEvaluationProcessServiceImpl implements MajorEvaluationProcessService, MajorEvaluationUserTypeConstant {
    @Resource
    ...
    public <T, F> void evaluationProcessPrincipalUpload(UserMessageDTO<T, F> userMessageDTO) {
        ...
        if (userType.contains(ARTS_EXPERT)) {
            informUsers = userMessageMapper.getAllUsersByRole(ARTS_EXPERT);
        ...
        } else if (userType.contains(SCIENCES_EXPERT)) {
            informUsers = userMessageMapper.getAllUsersByRole(SCIENCES_EXPERT);
        }
        ...
        userMessageService.insertBatchMessage(informUsers.stream().map(userId -> {
            String message = majorEvaluationDTO.getCollege() + "学院" + majorEvaluationDTO.getMajorName() + "专业正在进行专业评审，专业负责人" + senderUserDTO.getRealName() + "+已经上传自评材料，邀请您进行评审。";
    ...
    public <T, F> void evaluationProcessSendBackMaterial(UserMessageDTO<T, F> userMessageDTO) {
        ...
        String message = majorEvaluationDTO.getCollege() + "学院" + majorEvaluationDTO.getMajorName() + "专业评审中，" + sender.getRealName() + "评审专家对您的自评材料打回，请您重新上传材料";

        userMessageService.insertOneMessage(new UserMessage(1, 0, "系统", "提醒", (Long) userMessageDTO.getObjectId(), "专业评审流程",
                "", receiverId, message));
    ...
    public <T, F> void evaluationProcessExpertSubmit(UserMessageDTO<T, F> userMessageDTO) {
        ...
        List<Integer> informUsers = null;
        if (userType.contains(ARTS_EXPERT_LEADER)) {
            informUsers = userMessageMapper.getAllUsersByRole(ARTS_EXPERT_LEADER);
        ...
        } else if (userType.contains(SCIENCES_EXPERT_LEADER)) {
            informUsers = userMessageMapper.getAllUsersByRole(SCIENCES_EXPERT_LEADER);
        }
        ...
        userMessageService.insertBatchMessage(informUsers.stream().map(receiverId -> {
            String message = majorEvaluationDTO.getCollege() + "学院" + majorEvaluationDTO.getMajorName() + "专业评审中已经进行专家评审，邀请您进行评审。";
    ...
    public <T, F> void evaluationProcessExpertLeaderSubmit(UserMessageDTO<T, F> userMessageDTO) {
        ...
        userMessageService.insertBatchMessage(informUsers.stream().map(receiverId -> {
            String message = majorEvaluationDTO.getCollege() + "学院" + majorEvaluationDTO.getMajorName() + "专业评审中已经进行专家组长评审，邀请您进行评审。";
            return new UserMessage(1, 0, "系统", "提醒", (Long) userMessageDTO.getObjectId(), "专业评审流程",
                    "", receiverId, message);
        }).collect(Collectors.toList()));
    }
